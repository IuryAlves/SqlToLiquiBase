...
def create_sequence():
    ...
    with open('create sequence.sql', 'r') as _file:
    ...
    for sequence in sequences:
        if sequence:
            if sequence.find('CREATE GENERATOR ') > -1:
                ...
                with open(DIRETORIO + sequenceName.lower() + '.xml', 'w') as arq:
                    arq.write('<!--ORIGINAL SQL: ' +
                              sequence.replace('\n', '') + '-->')
                    arq.write(_globals.changelog)
                    arq.write('\n\t<changeSet id="1-' +
                              sequenceName + '" author="' + USUARIO + '">')
                    arq.write('\n\t\t<createSequence catalogName="' +
                              schema + '"')
                ...
                with open(DIRETORIO + sequenceName.lower() + '.xml', 'w') as arq:
                    ...
                    if pos > -1:
                        arq.write('\n\t\t\tstartValue="' +
                                  sequence[sequence.find(' '):pos + 12] + '"/>')
                    pos = sequence.find(' INCREMENT BY ')
                    if pos > -1:
                        arq.write('\n\t\t\tincrementBy="' +
                                  sequence[sequence.find(' '):pos + 14] + '"/>')
                    pos = sequence.find(' MINVALUE ')
                    if pos > -1:
                        arq.write('\n\t\t\tminValue="' +
                                  sequence[sequence.find(' '):pos + 10] + '"/>')
            ...
            insertSequence += '<include file="' + \
                DIRETORIO + sequenceName + '.xml"/>\n'
        else:
            print('vazio')
    print('\n----------------------------------------\n' +
